...
__logger__ = logging.getLogger("lcil.examples.cartpole.cert_result_test")


@dataclass(frozen=True)
class RetestCertResultScriptConfig(ArgumentParserConfig):
    lyapunov_dir: str | None = config_field(
        help="Defaults to the latest saved result under results/cartpole.",
    )
    ...
    method: Literal["ray_shooting", "monte_carlo"] = config_field(
        default="ray_shooting",
        help="Level set measure estimation method ('ray_shooting' or 'monte_carlo').",
    )
...
def main() -> None:
    ...
    first_cfg = script_configs[0]
    output_root = (
        Path(first_cfg.lyapunov_dir).resolve().parent.parent
        if first_cfg.lyapunov_dir is not None
        else Path(".").resolve()
    )

    with LevelSetMetricsWriter(output_root=output_root, method=first_cfg.method) as metrics_writer:
