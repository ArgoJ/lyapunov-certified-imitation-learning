...
def _lookup_config_value(config: Any, field_name: str) -> Any:
    ...
    for part in parts:
        if isinstance(obj, (tuple, list)) and part.isdigit():
            obj = obj[int(part)]
        elif not hasattr(obj, part):
            raise AttributeError(f"Config object has no field '{part}' in path '{field_name}'.")
        else:
...
def _get_alias_name(field_path: str, aliases: Mapping[str, str]) -> str:
    display_name = field_path
    if "." in field_path and field_path.split(".")[0].isdigit():
        display_name = field_path.split(".", 1)[1]
    return aliases.get(field_path, display_name)
...
def describe_grid_search_run(
    ...
    *,
    ...
    for field_name in include_fields:
        ...
        final_name = _get_alias_name(field_name, aliases)
        parts.append(f"{final_name}: {_format_display_value(value)}")
...
def _iter_config_items(config: Any) -> list[tuple[str, Any]]:
    if isinstance(config, (tuple, list)):
        ...
        for index, item in enumerate(config):
            sub_items = _iter_config_items(item)
            for sub_name, sub_value in sub_items:
                items.append((f"{index}.{sub_name}", sub_value))
        return items
...
def _resolve_configs(*configs: Sequence[ConfigT] | ConfigT) -> list[tuple[ConfigT, ...]] | list[ConfigT]:
    """Iterate over the Cartesian product of multiple config sequences."""
    if not configs:
        return []

    if len(configs) == 1:
        if isinstance(configs[0], Sequence) and not is_dataclass(configs[0]):
...
def _collect_display_aliases(config: Any) -> dict[str, str]:
    ...
    for field_info in fields(config):
        display_alias = field_info.metadata.get("display_alias")
        if isinstance(display_alias, str) and display_alias:
            aliases[field_info.name] = display_alias
    return aliases
...
def build_grid_search_run_name(
    ...
    *,
    ...
    for field_name in include_fields:
        ...
        final_name = _get_alias_name(field_name, aliases)
        segments.append(
            f"{_sanitize_segment(final_name)}_{_format_name_value(value)}"
        )
