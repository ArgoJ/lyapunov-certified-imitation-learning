...
class ClosedLoopLyapunovConditionVerifier(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("kappa", th.tensor(float(kappa), dtype=th.float32))
        self.register_buffer("rho", th.tensor(float(rho), dtype=th.float32))

    def set_rho(self, rho: float) -> None:
        """In-place update of rho to avoid retracing the AutoLiRPA graph."""
        self.rho.copy_(th.tensor(float(rho), dtype=self.rho.dtype, device=self.rho.device))

    def set_kappa(self, kappa: float) -> None:
        """In-place update of kappa to safely change the condition margin at runtime."""
        self.kappa.copy_(th.tensor(float(kappa), dtype=self.kappa.dtype, device=self.kappa.device))
